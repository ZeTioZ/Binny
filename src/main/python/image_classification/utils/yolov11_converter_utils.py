...
def convert_image_box_outputs(outputs) -> list[Box]:
	"""
	...
	for box_obj in outputs[0].boxes:
		...
		boxes.append(Box(position_1, position_2, cls=outputs[0].names[int(box_obj.cls)], conf=round(box_obj.conf[0].item(), 2)))
	return boxes